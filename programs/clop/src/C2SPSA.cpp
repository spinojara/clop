...
// Constructor
/////////////////////////////////////////////////////////////////////////////
C2SPSA::C2SPSA(CResults &results, double a, double A, double c, double alpha, double gamma, double d, double w)
    : CObserver(results), Dimensions(results.GetDimensions()), a(a), A(A), c(c), alpha(alpha), gamma(gamma), d(d), w(w),
      vTheta(Dimensions), vDelta(Dimensions), vDeltaTilda(Dimensions), vSample(Dimensions), vGHat(Dimensions),
      vDeltaGHat(Dimensions), vHHat(Dimensions * Dimensions), vHBar(Dimensions * Dimensions),
...
void C2SPSA::OnOutcome(int i) {
	...
	if ((i & 3) == 3) {
		for (int i = Dimensions; --i >= 0;) {
			...
			vDeltaGHat[i] =
				((ty[yplustilda] - ty[yplus]) - (ty[yminustilda] - ty[yminus])) / (ctilda_k * vDeltaTilda[i]);
		}

		for (int i = Dimensions; --i >= 0;)
			for (int j = Dimensions; --j >= 0;)
				vHHat[i * Dimensions + j] = 0.5 * (vDeltaGHat[i] / (2.0 * c_k * vDelta[j]) +
