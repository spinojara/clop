...
CBAST::CBAST(CResults &results, double Exploration)
    : CObserver(results), area(results.GetDimensions()), Dimensions(results.GetDimensions()), Exploration(Exploration),
      pnodeRoot(0) {
	OnReset();
}
...
const double *CBAST::NextSample(int i) {
	...
	while (true) {
		...
		if (pnode->tpnodeChild[0]) {
			...
			if (n1 * (v0 + Exploration * std::sqrt(LogT * n0)) > n0 * (v1 + Exploration * std::sqrt(LogT * n1)))
...
bool CBAST::MaxParameter(double vMax[]) const {
	...
	while (pnode->Victories && pnode->tpnodeChild[0] && pnode->tpnodeChild[0]->Games && pnode->tpnodeChild[1]->Games &&
