...
// Constructor
/////////////////////////////////////////////////////////////////////////////
CCrossEntropy::CCrossEntropy(CResults &results, double Smoothing, double DynamicSmoothing, bool fDistributionSmoothing,
			     int PopulationSize, int SelectionSize, int InitialBatchSize, double BatchGrowth, bool fDiagonal)
    : CObserver(results), Dimensions(results.GetDimensions()), Smoothing(Smoothing), DynamicSmoothing(DynamicSmoothing),
      fDistributionSmoothing(fDistributionSmoothing), PopulationSize(PopulationSize), SelectionSize(SelectionSize),
      InitialBatchSize(InitialBatchSize), BatchGrowth(BatchGrowth), fDiagonal(fDiagonal), vMean(Dimensions),
      vVariance(Dimensions * Dimensions), vOldMean(Dimensions), vOldVariance(Dimensions * Dimensions) {
	...
	if (Generation == 1) {
		...
		for (int i = int(vPopulation.size()); --i >= 0;)
			if (vPopulation[i] < vPopulation[iMax])
				iMax = i;
		std::copy(vPopulation[iMax].vParameters.begin(), vPopulation[iMax].vParameters.end(), vMax);
	}
	else
