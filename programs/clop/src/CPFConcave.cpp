...
CPFConcave::CPFConcave(int Dimensions, int Resolution)
    : CParametricFunction(Dimensions, 1 + Dimensions * Resolution * 2), Resolution(Resolution), vCenter(Resolution) {
...
// Compute monomials
/////////////////////////////////////////////////////////////////////////////
void CPFConcave::GetMonomials(const double *restrict vx, double *restrict vMonomial) const {
...
// Gradient
/////////////////////////////////////////////////////////////////////////////
void CPFConcave::GetGradient(const double *restrict vParam, const double *restrict vx, double *restrict vG) const {
	for (int i = Dimensions, p = Parameters; --i >= 0;) {
		vG[i] = 0.0;
		for (int j = Resolution; --j >= 0;) {
...
// Prior gradient
/////////////////////////////////////////////////////////////////////////////
void CPFConcave::GetPriorGradient(const double *restrict vParam, double *restrict vGradient) const {
	for (int p = Parameters; --p > 0;) {
		double x = MakePositive(vParam[p]);
...
// Prior Hessian
/////////////////////////////////////////////////////////////////////////////
void CPFConcave::GetPriorHessian(const double *restrict vParam, double *restrict vHessian) const {
	...
	for (int p = Parameters; --p > 0;) {
		...
		vHessian[p * (Parameters + 1)] = GetPriorStrength() * ((xp * xp) / (x * x) - xpp * (1.0 / x - 1.0));
	}
