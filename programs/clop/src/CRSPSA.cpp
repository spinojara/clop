...
// Constructor
/////////////////////////////////////////////////////////////////////////////
CRSPSA::CRSPSA(CResults &results, int BatchSize, double BatchGrowth, double EtaPlus, double EtaMinus, double DeltaMin,
	       double DeltaMax, double Delta0, double Rho)
    : CObserver(results), Dimensions(results.GetDimensions()), BatchSize(BatchSize), BatchGrowth(BatchGrowth), EtaPlus(EtaPlus),
      EtaMinus(EtaMinus), DeltaMin(DeltaMin), DeltaMax(DeltaMax), Delta0(Delta0), Rho(Rho), vTheta(Dimensions),
      vDelta(Dimensions), vPerturbation(Dimensions), vPreviousGradient(Dimensions), vCurrentGradient(Dimensions),
      vSample(Dimensions) {
	OnReset();
}
