...
double CRegression::ResultProbability(double Rating, COutcome outcome) const {
	...
	case COutcome::Loss:
		return CLogistic::f(-Rating - DrawRating);
	case COutcome::Draw:
		return CLogistic::f(Rating - DrawRating) * CLogistic::f(-Rating - DrawRating);
	}
	return 1.0;
}
...
double CRegression::GetProba(int k, const double *vParam) const {
	return ResultProbability(pf.GetValue(vParam, GetSample(k)), results.GetOutcome(k));
}
...
void CRegression::UpdateLogP() {
	...
	for (int k = Samples; --k >= 0;) {
		...
		L += vsd[k].Weight * (tLogP[0] * vsd[k].tCount[0] + tLogP[1] * vsd[k].tCount[1] + tLogP[2] * vsd[k].tCount[2]);
	}
	...
	for (int k = Samples; --k >= 0;) {
		...
		for (int p1 = pf.GetParameters(); --p1 >= 0;)
			for (int p2 = pf.GetParameters(); --p2 >= p1;)
				vHessian[p1 * pf.GetParameters() + p2] += TotalMul * vMonomial[p1] * vMonomial[p2];
	}
...
void CRegression::EnsureState(int Flags) {
	while (Flags & ~State) {
		...
		if ((Flags & ~State) & S_Cholesky) {
			...
			fCholesky = CMatrixOperations::Cholesky(&vHessian[0], &vCholesky[0], pf.GetParameters());
			State |= S_Cholesky;
		}

		if ((Flags & ~State) & S_CholeskyInverse) {
			EnsureState(S_Cholesky);
			CMatrixOperations::Inverse(&vCholesky[0], &vCholeskyInverse[0], pf.GetParameters());
...
// Constructor
/////////////////////////////////////////////////////////////////////////////
CRegression::CRegression(CResults &results, const CParametricFunction &pf)
    : CObserver(results), pf(pf), DrawRating(0), vParamMAP(pf.GetParameters()), vMonomial(pf.GetParameters()),
      vGradient(pf.GetParameters()), vHessian(pf.GetParameters() * pf.GetParameters()),
      vCholesky(pf.GetParameters() * pf.GetParameters()), vCholeskyInverse(pf.GetParameters() * pf.GetParameters()),
      RefreshRate(0.0), LocalizationHeight(3.0), LocalizationPower(0.0), MaxWeightIterations(0), fAutoLocalize(true),
...
void CRegression::OnSample() {
	...
	if (k > 0 && std::equal(GetSample(k - 1), GetSample(k - 1) + pf.GetDimensions(), GetSample(k))) {
		vsd[k] = vsd[k - 1];
		vsd[k].Replications++;
	}
	else {
...
/////////////////////////////////////////////////////////////////////////////
void CRegression::GaussianSample(CRandom<unsigned> &rnd, std::vector<double> &vParam) {
...
/////////////////////////////////////////////////////////////////////////////
void CRegression::MCMCSample(CRandom<unsigned> &rnd, std::vector<double> &vParam, int n) {
	OnRefresh();
...
/////////////////////////////////////////////////////////////////////////////
double CRegression::GetWeight(const double *vx) const { return std::exp(GetLogWeight(vx)); }
...
void CRegression::GetMeanAndDeviation(double &Mean, double &Deviation) {
	...
	const double Delta =
		(tWC[1] - tWC[0]) * (tWC[1] - tWC[0]) + 4 * GammaD * GammaD * (tWC[0] + tWC[2]) * (tWC[1] + tWC[2]);
	const double Gamma = (tWC[1] - tWC[0] + std::sqrt(Delta)) / (2 * GammaD * (tWC[0] + tWC[2]));
	Mean = std::log(Gamma);
	...
	const double x = (tWC[1] + tWC[2]) * PWin * (1.0 - PWin) + (tWC[0] + tWC[2]) * PLoss * (1.0 - PLoss);
...
void CRegression::LocalizationIteration() {
	if (LocalizationHeight > 0 && tCount[COutcome::Loss] > 1 && tCount[COutcome::Win] > 1) {
		//
		...
		weighting.Radius = Deviation * LocalizationHeight * std::pow(double(Samples), LocalizationPower);
		weighting.Mean = Mean;
...
/////////////////////////////////////////////////////////////////////////////
void CRegression::GetPosteriorInfo(const double *vLocation, double &Rating, double &Variance) {
