...
// Constructor
/////////////////////////////////////////////////////////////////////////////
CSPVOptimal::CSPVOptimal(const CRegression &reg, CDFVariance &dfvar, int Trials)
    : reg(reg), dfvar(dfvar), Trials(Trials), v(reg.GetPF().GetDimensions()), vTrial(reg.GetPF().GetDimensions()), nTotal(0),
      spCG(reg), spFail(reg) {}
...
const double *CSPVOptimal::NextSample(int i) {
	...
	//
	if (reg.GetCount(COutcome::Loss) >= dfvar.GetMinSamples() && reg.GetCount(COutcome::Win) >= dfvar.GetMinSamples()) {
