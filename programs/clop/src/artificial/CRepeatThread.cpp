...
// Constructor
/////////////////////////////////////////////////////////////////////////////
CRepeatThread::CRepeatThread(CRepeatThreads &rts, CArtificialExperiment &artexp)
    : rts(rts), artexp(artexp), Seed(0), vMax(artexp.problem.GetDimensions()), vOptimal(artexp.problem.GetDimensions()) {
...
void CRepeatThread::operator()() {
	while (rts.Dispatch(*this)) {
		...
		for (int i = int(rts.vpcpd.size()); --i >= 0;) {
			...
			const int Min = i + 1 < int(rts.vpcpd.size()) ? rts.vpcpd[i + 1]->GetSamples() : 0;
			...
			// Compute error
			//
			double Error = artexp.problem.GetOptimalProba() - artexp.problem.GetProba(&vMax[0]);
