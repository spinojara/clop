...
// Add style
/////////////////////////////////////////////////////////////////////////////
void CTikZ::AddStyle(const std::string &s, float R, float G, float B, const std::string &sStyle) {
...
/////////////////////////////////////////////////////////////////////////////
CTikZ::CTikZ()
    : pout(&std::cout), Scale(1.0), Radius(0.007), Tick(0.02), CircleN(2), fGrayscale(false), SplineSamples(1000),
      SplineD(0.001), fResults(false) {
...
// Set style
/////////////////////////////////////////////////////////////////////////////
void CTikZ::SetStyle(const std::string &s, float R, float G, float B, const std::string &sStyle) {
...
void CTikZ::Prolog() const {
	*pout << "\\begin{tikzpicture}\n";
	...
	//
	for (std::map<std::string, CStyle>::const_iterator i = mapStyle.begin(); i != mapStyle.end(); i++) {
		//
		// Color
		//
...
// Epilog
/////////////////////////////////////////////////////////////////////////////
void CTikZ::Epilog() const { *pout << "\\end{tikzpicture}\n"; }
	...
	for (unsigned i = 0; i < results.GetSamples(); i++) {
		...
		if (preg) {
			...
			*pout << "\\pgfusepathqfillstroke\n";
			...
			if (results.GetOutcome(i) == COutcome::Win || i == results.GetSamples() - 1 ||
