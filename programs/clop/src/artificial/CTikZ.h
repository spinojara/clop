...
class CTikZ // tikz
{
	...
	struct CStyle {
		...
		CStyle(float R, float G, float B, const std::string &sStyle) : sStyle(sStyle), col(CColor(R, G, B)) {}
	};

	std::map<std::string, CStyle> mapStyle;

	void AddStyle(const std::string &s, float R, float G, float B, const std::string &sStyle);
	...
	void SetStyle(const std::string &s, float R, float G, float B, const std::string &sStyle);
