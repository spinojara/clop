...
// Constructor
/////////////////////////////////////////////////////////////////////////////
CTikZOneD::CTikZOneD() : fTrue(false), fPosterior(false), fMAP(false), fExpected(false), fConfidence(false), fLCB(false) {
	Scale = 5.0;
	Tickp = 1.0 + Tick;
	Tickm = 1.0 - Tick;
}
...
/////////////////////////////////////////////////////////////////////////////
void CTikZOneD::EndClip() const { *pout << "\\end{scope}\n"; }
...
// Posterior Samples
/////////////////////////////////////////////////////////////////////////////
void CTikZOneD::Posterior(int Samples, CRegression &reg, CRandom<unsigned> &rnd) {
	fPosterior = true;
...
void CTikZOneD::Frame(int Samples) const {
	...
	*pout << Scale << "," << Scale
		 ...
		 "\\scriptsize\n"
		 "\\draw (-"
	      << Scale << ",-" << Scale * Tick << ") -- (-" << Scale
	      << ",0);\n"
		 "\\draw (+"
	      << Scale << ",-" << Scale * Tick << ") -- (+" << Scale
	      << ",0);\n"
		 "\\path (-"
	      << Scale << ",-" << Scale * Tick << ") -- (+" << Scale << ",-" << Scale * Tick
	      << ")\n"
		 ...
		 "\\draw (-"
	      ...
	      << ",0);\n"
		 "\\draw (-"
	      << Scale * Tickp << "," << Scale << ") -- (-" << Scale << "," << Scale
	      << ");\n"
		 "\\path (-"
	      << Scale * Tickp << ",0) -- (-" << Scale * Tickp << "," << Scale
	      << ")\n"
		 "      node[left,midway] {$P$}\n"
		 "      node[left,pos=0] {0}\n"
		 "      node[left,pos=1] {1};\n";
