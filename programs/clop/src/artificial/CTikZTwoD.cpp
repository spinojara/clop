...
// Constructor
/////////////////////////////////////////////////////////////////////////////
CTikZTwoD::CTikZTwoD() : ContourResolution(400) {
	Scale = 2.5;
	Radius = 0.02;
	Tick = 0.04;
	CircleN = 2;
...
/////////////////////////////////////////////////////////////////////////////
void CTikZTwoD::EndClip() const { *pout << "\\end{scope}\n"; }
...
// Posterior Samples
/////////////////////////////////////////////////////////////////////////////
void CTikZTwoD::Posterior(int Samples, CRegression &reg, CRandom<unsigned> &rnd) {
	...
	*pout << Scale << "," << Scale << ");\n";
	if (fLabels) {
		...
		*pout << "\\draw (-" << Scale << ",-" << Scale * (1.0 + Tick) << ") -- (-" << Scale << "," << -Scale
		      << ");\n"
			 "\\draw (+"
		      << Scale << ",-" << Scale * (1.0 + Tick) << ") -- (+" << Scale << "," << -Scale
		      << ");\n"
			 "\\path (-"
		      << Scale << ",-" << Scale * (1.0 + Tick) << ") -- (+" << Scale << ",-" << Scale * (1.0 + Tick)
		      << ")\n"
		...
		*pout << "\\draw (-" << Scale * (1.0 + Tick) << ",-" << Scale << ") -- (-" << Scale << "," << -Scale
		      << ");\n"
			 "\\draw (-"
		      << Scale * (1.0 + Tick) << ",+" << Scale << ") -- (-" << Scale << "," << +Scale
		      << ");\n"
			 "\\path (-"
		      << Scale * (1.0 + Tick) << ",-" << Scale << ") -- (-" << Scale * (1.0 + Tick) << "," << Scale
		      << ")\n"
