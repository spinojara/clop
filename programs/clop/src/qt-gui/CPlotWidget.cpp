...
// Constructor
/////////////////////////////////////////////////////////////////////////////
CPlotWidget::CPlotWidget(QWidget *parent) : QWidget(parent), mw(0), backgroundColor(195, 112, 255) {
...
void CPlotWidget::paintEvent(QPaintEvent *event) {
	...
	if (mw && mw->pexperiment) {
		...
		for (unsigned i = first; i < last; i++) {
			...
			//
			bool fMerge = false;
			if (mw->ui->mergeReplicationsCheckBox->isChecked() && results.GetOutcome(i) != COutcome::Unknown &&
			    results.GetOutcome(i) != COutcome::InProgress) {
				while (i < last - 1 && results.GetOutcome(i + 1) != COutcome::Unknown &&
			...
			if (fMerge) {
				...
				const int c = int(255.0 * (sd.tCount[COutcome::Win] + 0.5 * sd.tCount[COutcome::Draw]) /
						  sd.GetCount());
				col = QColor(c, c, c);
			}
			else
			...
			if (mw->ui->sampleWeightCheckBox->isChecked() && results.GetOutcome(i) != COutcome::InProgress) {
				double x =
					mw->pexperiment->reg.GetSampleData(mw->pexperiment->reg.GetSampleData(i).Index).Weight;
				if (x > 1.0)
					x = 1.0;
				int Weight = x * 256;
				if (Weight == 0)
				...
				col.setRgb((r * Weight + r0 * (256 - Weight)) >> 8, (g * Weight + g0 * (256 - Weight)) >> 8,
					   (b * Weight + b0 * (256 - Weight)) >> 8, a0);
			}
			...
			// Plot point
			//
			painter.fillRect(xx - PointSize / 2, yy - PointSize / 2, PointSize, PointSize, col);
		}
	}
}
