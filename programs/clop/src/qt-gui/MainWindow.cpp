...
// Constructor
/////////////////////////////////////////////////////////////////////////////
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow), pes(0), pexperiment(0) {
	ui->setupUi(this);
...
void MainWindow::loadFile(const QString &fileName) {
	...
	//
	connect(&pexperiment->rexp, SIGNAL(Finished()), SLOT(onRexpReady()));
	connect(&pexperiment->rexp, SIGNAL(Started()), SLOT(onRexpRunning()));

	//
	// Connections for GUI update
	//
	updateConnections();
	connect(&pexperiment->rexp, SIGNAL(Sample()), SLOT(updateCounter()));
	...
	connect(&pexperiment->rexp, SIGNAL(StartLoading(int)), &progress, SLOT(setMaximum(int)));
	connect(&pexperiment->rexp, SIGNAL(LoadingProgress(int)), &progress, SLOT(setValue(int)));
	...
	ui->eigenTable->setVerticalHeaderLabels(QStringList() << "Eigenvalue"
							      << "" << slNames);
	...
	ui->maxTable->setVerticalHeaderLabels(QStringList() << slNames << " "
							    << "95% UCB"
							    << "Elo"
							    << "95% LCB"
							    << " "
	...
	ui->winRateTable->setHorizontalHeaderLabels(QStringList() << "All"
								  << "Weighted"
								  << "Central");
	ui->winRateTable->setVerticalHeaderLabels(QStringList() << "Wins"
								<< "Draws"
								<< "Losses"
								<< "Games"
								<< " "
...
void MainWindow::updateConnections() {
	if (pexperiment) {
		if (ui->action_Automatic_GUI_update->isChecked()) {
			connect(&pexperiment->rexp, SIGNAL(Message(std::string)), SLOT(onRexpMessage(std::string)));
			connect(&pexperiment->rexp, SIGNAL(Sample()), SLOT(updatePlot()));
			connect(&pexperiment->rexp, SIGNAL(Result()), SLOT(updateAll()));
		}
		else {
			disconnect(&pexperiment->rexp, SIGNAL(Message(std::string)), this, SLOT(onRexpMessage(std::string)));
			disconnect(&pexperiment->rexp, SIGNAL(Sample()), this, SLOT(updatePlot()));
			disconnect(&pexperiment->rexp, SIGNAL(Result()), this, SLOT(updateAll()));
		}
	}
}
...
bool MainWindow::checkRunning() {
	if (state == Running || state == Waiting)
		return (QMessageBox::warning(this, "Warning", "Experiment is running. Do you really wish to stop it now?",
...
/////////////////////////////////////////////////////////////////////////////
{
	...
	QString fileName = QFileDialog::getOpenFileName(this, "Open Experiment", 0, "CLOP Files (*.clop *.qlr)");
	if (!fileName.isNull()) {
		deleteExperiment();
		loadFile(fileName);
	}
...
void MainWindow::updateStatistics() {
	if (pexperiment) {
		...
		//
		updateWinRate(0, pexperiment->reg.GetCount(COutcome::Win), pexperiment->reg.GetCount(COutcome::Draw),
		...
		updateWinRate(1, tCount[COutcome::Win], tCount[COutcome::Draw], tCount[COutcome::Loss]);
		updateWinRate(2, tCentralCount[COutcome::Win], tCentralCount[COutcome::Draw], tCentralCount[COutcome::Loss]);
	}
}
...
// About Qt
/////////////////////////////////////////////////////////////////////////////
void MainWindow::on_action_About_Qt_triggered() { QMessageBox::aboutQt(this); }
