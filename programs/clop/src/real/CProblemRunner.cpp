...
// Constructor
/////////////////////////////////////////////////////////////////////////////
CProblemRunner::CProblemRunner(int id, const CRealProblem &problem, const std::string &sProcessor)
    : id(id), problem(problem), sProcessor(sProcessor) {
	connect(&proc, SIGNAL(error(QProcess::ProcessError)), SLOT(OnError(QProcess::ProcessError)));
	connect(&proc, SIGNAL(finished(int, QProcess::ExitStatus)), SLOT(OnFinished(int, QProcess::ExitStatus)));
}
...

{
	...
	for (std::list<std::string>::const_iterator i = problem.lsScriptParam.begin(); i != problem.lsScriptParam.end(); ++i)
...
void CProblemRunner::OnFinished(int exitCode, QProcess::ExitStatus) {
	if (!fError) {
		...
		if (proc.getChar(&result))
			switch (result) {
			case 'W':
				emit Outcome(id, CurrentSeed, COutcome::Win);
				return;
			case 'L':
				emit Outcome(id, CurrentSeed, COutcome::Loss);
				return;
			case 'D':
				emit Outcome(id, CurrentSeed, COutcome::Draw);
				return;
			default:
				proc.ungetChar(result);
			}
