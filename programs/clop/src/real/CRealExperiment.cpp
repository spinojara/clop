...
#define LOG_ID(x)                                                       \
	LOG("id = " << id << " (" << vpr[id]->GetProcessorName() << ")" \
		    << "; Seed = " << Seed << "; " << x)
...
void CRealExperiment::ReplayBackup(std::ifstream &ifs) {
	...
	LOG("Backup done.");
	LOG("Samples = " << Samples << "; Results = " << Results << "; Mismatches = " << Mismatches << '.');
}

/////////////////////////////////////////////////////////////////////////////
// Constructor
/////////////////////////////////////////////////////////////////////////////
CRealExperiment::CRealExperiment(const CRealProblem &problem, CSamplingPolicy &sp, CResults &results, const std::string &sName,
				 int Replications, bool fStopOnError)
    : CObserver(results), problem(problem), sp(sp), results(results), sName(sName), Replications(Replications),
      fStopOnError(fStopOnError), fStop(false), RunningProcessors(0) {
...
void CRealExperiment::StartLogs() {
	...
	//
	{
		...
		if (File.exists()) {
			...
			LOG("Renaming " << datName.toStdString() << " to " << backupName.toStdString());
...
/////////////////////////////////////////////////////////////////////////////
void CRealExperiment::SoftStop() { fStop = true; }
...
void CRealExperiment::AddProcessor(const std::string &s) {
	...
	vpr.push_back(ppr);
	connect(ppr, SIGNAL(Outcome(int, int, COutcome)), SLOT(OnPROutcome(int, int, COutcome)));
	connect(ppr, SIGNAL(Error(int, int)), SLOT(OnPRError(int, int)));
}
