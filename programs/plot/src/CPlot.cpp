...
void CPlot::UpdateTransform() {
	...
	//
	{
		tTransform[3] = tTransform[7] * tTransform[2] - tTransform[8] * tTransform[1];
		tTransform[4] = tTransform[8] * tTransform[0] - tTransform[6] * tTransform[2];
		tTransform[5] = tTransform[6] * tTransform[1] - tTransform[7] * tTransform[0];
	}
}
...
// Conic projection
/////////////////////////////////////////////////////////////////////////////
void CPlot::Transform(double x, double y, double z, double &x2, double &y2) const {
	x -= tObserver[0];
	y -= tObserver[1];
	z -= tObserver[2];
...
// Constructor
/////////////////////////////////////////////////////////////////////////////
CPlot::CPlot(const CSurface &surfInit) : surf(surfInit) {
...
/////////////////////////////////////////////////////////////////////////////
void CPlot::Projection(double x, double y, double &x2, double &y2) const { Transform(x, y, surf.GetValue(x, y), x2, y2); }
	...
	for (int i = 2; --i >= 0;)
		for (int j = 2; --j >= 0;)
			for (int k = 2; --k >= 0;) {
				double x;
				double y;
				Transform(xMin + double(i) * (xMax - xMin), yMin + double(j) * (yMax - yMin),
					  zMin + double(k) * (zMax - zMin), x, y);
				if (x < x0)
					x0 = x;
				if (y < y0)
					y0 = y;
