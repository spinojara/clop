...
// Command strings
////////////////////////////////////////////////////////////////////////////
const char *const CPlotCUI::tszCommands[] = {"?",     "obs",	 "target",   "ranges", "dist",	 "grid",  "line",
					     "dmax",  "visible", "exclude",  "smooth", "dots",	 "steps", "plotx",
					     "ploty", "gnuplot", "bbox",     "axis",   "lscale", "label", "project",
					     "write", "contour", "contours", 0};
...
CPlotCUI::CPlotCUI(CPlot &plotInit, CConsoleUI *pcui, int openmode)
    : CConsoleUI(pcui, openmode), plot(plotInit), GridSamples(50), LineSamples(500), Exclude(0), fSmooth(1), fDots(0), Steps(1),
      DMax(0.01), LabelScale(1.0) {}

/////////////////////////////////////////////////////////////////////////////
// Local prompt
/////////////////////////////////////////////////////////////////////////////
void CPlotCUI::PrintLocalPrompt(std::ostream &out) { out << "CPlot"; }

/////////////////////////////////////////////////////////////////////////////
// Plot a line (with hidden parts) (should use better method)
/////////////////////////////////////////////////////////////////////////////
void CPlotCUI::PlotLine(double x0, double y0, double z0, double x1, double y1, double z1, std::ostream &out) const {
	double t = 0;
	...
	while (t < 1.0) {
		...
		if (!fVisible && f)
			...
			double x20;
			double y20;
			plot.Transform(x0 + t0 * (x1 - x0), y0 + t0 * (y1 - y0), z0 + t0 * (z1 - z0), x20, y20);
...
////////////////////////////////////////////////////////////////////////////
double CPlotCUI::VisibilityChange(double x0, double y0, int f0, double x1, double y1, int f1) const {
	double tMin = 0.0;
	double tMax = 1.0;
...
// Plot a part of line
////////////////////////////////////////////////////////////////////////////
void CPlotCUI::psline(int n, const double *px, const double *py, const int *pfDot, std::ostream &out) const {
...
// Plot a curve
////////////////////////////////////////////////////////////////////////////
void CPlotCUI::curve(int n, const double *px, const double *py, double DMax, std::ostream &out) const {
	CDiscretizedLine dl;
	for (int i = 0; i < n; i++)
...
// Plot an isoline
////////////////////////////////////////////////////////////////////////////
void CPlotCUI::PlotIsoLine(double x0, double y0, double x1, double y1, std::ostream &out) const {
...
// Parse a corner
////////////////////////////////////////////////////////////////////////////
void CPlotCUI::ParseCorner(const char *psz, double &x, double &y, double &z) const {
	if (psz[0] == '0')
		x = plot.GetXMin();
	else
		x = plot.GetXMax();
...
void CPlotCUI::Contour(std::ostream &out, double Level, int W, int H) const {
	...
	contour.SetBounds(plot.GetXMin(), plot.GetXMax(), plot.GetYMin(), plot.GetYMax());
...
////////////////////////////////////////////////////////////////////////////
int CPlotCUI::ProcessCommand(const char *pszCommand, const char *pszParameters, std::istream &in, std::ostream &out) {
	...
	case IDC_Ranges: /////////////////////////////////////////////////////////
	{
		...
		std::istringstream(pszParameters) >> xMin >> xMax >> yMin >> yMax >> zMin >> zMax;
	...
	case IDC_PlotX: //////////////////////////////////////////////////////////
	{
		for (int i = 0; i <= GridSamples; i++)
			if (Exclude <= 0 || i % Exclude) {
				double x =
					plot.GetXMin() + (plot.GetXMax() - plot.GetXMin()) * (double(i) / double(GridSamples));
	...
	case IDC_PlotY: //////////////////////////////////////////////////////////
		for (int i = 0; i <= GridSamples; i++)
			if (Exclude <= 0 || i % Exclude) {
				double y =
					plot.GetYMin() + (plot.GetYMax() - plot.GetYMin()) * (double(i) / double(GridSamples));
