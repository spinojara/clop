...
class CTime // time
{
	...
	CTime(long l = 0) : lTime(l) {}
	CTime operator+=(long l) { return CTime(lTime += l); }
	CTime operator-=(long l) { return CTime(lTime -= l); }
	CTime operator*=(long l) { return CTime(lTime *= l); }
	CTime operator/=(long l) { return CTime(lTime /= l); }
	...
	//
	void Reset() { lTime = 0; }
	void Set(long lHours, long lMinutes, long lSeconds, long lHundredths) {
		lTime = lHundredths + lSeconds * 100 + lMinutes * 6000 + lHours * 360000L;
	}
